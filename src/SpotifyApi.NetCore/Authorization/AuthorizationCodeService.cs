...
namespace SpotifyApi.NetCore
{
    public class AuthorizationCodeService : IAuthorizationCodeService
    {
        ...
        public (string url, string state) RequestAuthorizationUrl(string userHash)
        {
            ...
            return
                (
                    $"{AuthHelper.AuthorizeUrl}/?client_id={_config["SpotifyApiClientId"]}&response_type=code&redirect_uri={_config["SpotifyAuthRedirectUri"]}&scope={_scopes}&state={StateHelper.EncodeState((userHash, state))}",
                    state
                );
        }

        public async Task<AuthorizationTokens> RequestTokens(string queryString)
        {
        ...
        public async Task<AuthorizationTokens> RequestTokens(string userHash, string state, string queryState, string code)
        {
            ...
            var tokens = await GetAuthorizationTokens(code);

            // Save the tokens in the auth table, delete the state and code
            // userAuth.SetPropertiesFromAuthCode(tokens);
            // await _data.Update(userAuth);

            return tokens;
        }

        public async Task<AuthorizationTokens> RequestTokens(IEnumerable<KeyValuePair<String, StringValues>> queryCollection)
        {
            throw new NotImplementedException();
